 ...
 *--------------------------------------------------------------------------------------------*/
import { HeaderProperty, HeaderPropertyType, KnownMediaType } from "@azure-tools/codemodel-v3";
...
import { DoNotFormatAttribute, FormatTableAttribute } from "../csharp-declarations";
import { Language, Schema as NewSchema, SchemaType, ObjectSchema, DictionarySchema } from "@autorest/codemodel";
import {
  VirtualProperty as NewVirtualProperty,
  getAllVirtualProperties as newGetAllVirtualProperties,
  getSchemaDetails,
} from "../../utils/schema";
...
export class ModelClass extends Class implements EnhancedTypeDeclaration {
  deserializeFromContainerMember(
    mediaType: KnownMediaType,
    container: ExpressionOrLiteral,
    serializedName: string,
    defaultValue: Expression,
  ): Expression {
    return this.featureImplementation.deserializeFromContainerMember(
      mediaType,
      container,
      serializedName,
      defaultValue,
  ...
  public get schema() {
    return this.featureImplementation.schema;
  }
  ...
  /* @internal */ validationEventListener: Parameter = new Parameter("eventListener", ClientRuntime.IEventListener, {
    description: `an <see cref="${ClientRuntime.IEventListener}" /> instance that will receive validation events.`,
  });
  ...
  constructor(
    namespace: Namespace,
    schemaWithFeatures: ObjectImplementation,
    state: State,
    objectInitializer?: DeepPartial<ModelClass>,
  ) {
  ...
  private nested(virtualProperty: NewVirtualProperty, internal: boolean): string {
    if (virtualProperty.accessViaProperty) {
      if (virtualProperty.accessViaProperty.accessViaProperty) {
        ...
        return `${getVirtualPropertyName(virtualProperty.accessViaMember)}.${this.nested(
          virtualProperty.accessViaProperty.accessViaProperty,
          internal,
        )}`;
      }
    }
  ...
  private accessor(virtualProperty: NewVirtualProperty, internal = false): string {
    if (virtualProperty.accessViaProperty) {
      const prefix = virtualProperty.accessViaProperty.accessViaProperty
        ? this.nested(virtualProperty.accessViaProperty.accessViaProperty, internal)
        : "";
      ...
      if (containingProperty && virtualProperty.accessViaMember) {
        ...
        return `((${
          virtualProperty.accessViaMember.originalContainingSchema.language.csharp?.fullInternalInterfaceName
        })${containingProperty.name}${prefix}).${getVirtualPropertyName(virtualProperty.accessViaMember)}`;
      }
    }
  ...
  private createProperties() {
    ...
    if (this.schema.language.csharp?.virtualProperties) {
      /* Owned Properties */
      for (const virtualProperty of values(
        <Array<NewVirtualProperty>>this.schema.language.csharp.virtualProperties.owned,
      )) {
        const actualProperty = virtualProperty.property;
        let n = 0;
        const decl = this.state.project.modelsNamespace.NewResolveTypeDeclaration(
          <NewSchema>actualProperty.schema,
          actualProperty.language.csharp?.required,
          this.state.path("schema"),
        );

        /* public property */
        const myProperty = new ModelProperty(
          virtualProperty.name,
          <NewSchema>actualProperty.schema,
          actualProperty.language.csharp?.required,
          actualProperty.serializedName,
          actualProperty.language.csharp?.description || "",
          this.state.path("properties", n++),
          {
            initializer: actualProperty.language.csharp?.constantValue
              ? typeof actualProperty.language.csharp.constantValue === "string"
                ? new StringExpression(actualProperty.language.csharp.constantValue)
                : new LiteralExpression(actualProperty.language.csharp.constantValue)
              : undefined,
          },
        );
        ...
        if (
          myProperty.getAccess !== Access.Public ||
          myProperty.setAccess !== Access.Public ||
          myProperty.set === undefined
        ) {
          ...
          this.add(
            new Property(
              `${getSchemaDetails(virtualProperty.originalContainingSchema)?.classImplementation?.fullName}.${
                virtualProperty.name
              }`,
              decl,
              {
      ...
      for (const virtualProperty of values(
        <Array<NewVirtualProperty>>this.schema.language.csharp.virtualProperties.inherited,
      )) {
        // so each parent property that is getting exposed
        // has to be accessed via the field in this.backingFields
        const csharp = getSchemaDetails(virtualProperty.accessViaSchema);
        const parentField = <BackingField>(
          this.backingFields.find((each) =>
            virtualProperty.accessViaSchema
              ? csharp?.classImplementation?.fullName === each.typeDeclaration.declaration
              : false,
          )
        );

        const propertyType = this.state.project.modelsNamespace.NewResolveTypeDeclaration(
          <NewSchema>virtualProperty.property.schema,
          !!virtualProperty.required,
          this.state,
        );
        const requiredPropertyType = this.state.project.modelsNamespace.NewResolveTypeDeclaration(
          <NewSchema>virtualProperty.property.schema,
          true,
          this.state,
        );
        const opsType = this.state.project.modelsNamespace.NewResolveTypeDeclaration(
          <NewSchema>virtualProperty.originalContainingSchema,
          false,
          this.state,
        );
      ...
      /* Inlined properties. */
      for (const virtualProperty of values(
        <Array<NewVirtualProperty>>this.schema.language.csharp.virtualProperties.inlined,
      )) {
        if (virtualProperty.private) {
          // continue;
        ...
        if (virtualProperty.accessViaProperty) {
          ...
          if (containingProperty) {
            ...
            const vp = new Property(virtualProperty.name, propertyType, {
              ...
              set:
                virtualProperty.property.language.csharp?.readOnly ||
                virtualProperty.property.language.csharp?.constantValue
                  ? undefined
                  : toExpression(
                      `${this.accessor(virtualProperty)} = value ${
                        virtualProperty.required ? "" : ` ?? ${requiredPropertyType.defaultOfType}`
                      }`,
                    ),
            });
            ...
            if (
              virtualProperty.private ||
              vp.getAccess !== Access.Public ||
              vp.setAccess !== Access.Public ||
              vp.set === undefined
            ) {
              this.add(
                new Property(
                  `${
                    getSchemaDetails(virtualProperty.originalContainingSchema)?.internalInterfaceImplementation
                      ?.fullName
                  }.${virtualProperty.name}`,
                  propertyType,
                  {
      ...
      /* Appended properties. */
      if (
        this.state.project.resourceGroupAppend &&
        this.state.project.azure &&
        this.schema.extensions &&
        this.schema.extensions["is-return-object"]
      ) {
  ...
  private additionalPropertiesType(aSchema: NewSchema): TypeDeclaration | undefined {
    const schema =
      aSchema.type === SchemaType.Dictionary
        ? aSchema
        : aSchema.type === SchemaType.Object
        ? (<ObjectSchema>aSchema).parents?.immediate?.find((s) => s.type === SchemaType.Dictionary)
        : undefined;
    if (schema) {
      const dictSchema = <DictionarySchema>schema;
      if (dictSchema.elementType.type === SchemaType.Any || dictSchema.elementType.type === SchemaType.AnyObject) {
  ...
  private addHeaderDeserializer() {
    ...
    let used = false;
    let i = 0;
    for (const headerProperty of values(avp).where(
      (each) =>
        each.property.language.csharp?.[HeaderProperty] === HeaderPropertyType.HeaderAndBody ||
        each.property.language.csharp?.[HeaderProperty] === HeaderPropertyType.Header,
    )) {
      used = true;
      ...
      const td = this.state.project.modelsNamespace.NewResolveTypeDeclaration(
      ...
      readHeaders.add(
        If(
          `${valueOf(headers)}.TryGetValues("${headerProperty.property.serializedName}", out var ${values}${i})`,
          `${t}.${headerProperty.name} = ${td.deserializeFromContainerMember(
  ...
  private appendResourceGroupName() {
    ...
    const idProperties = values(virtualProperties)
      .where((each) => each.name === "Id")
      .toArray();
    const resourceGroupNameProperties = values(virtualProperties)
      .where((each) => each.name === "ResourceGroupName")
      .toArray();
    ...
    if (idProperties.length == 1 && resourceGroupNameProperties.length === 0) {
      ...
      const decl = this.state.project.modelsNamespace.NewResolveTypeDeclaration(
        <NewSchema>actualResourceGroupProperty.schema,
        false,
        this.state.path("schema"),
      );
