...
import { SchemaResponse, Schema, isObjectSchema, SchemaType, StringSchema, ChoiceSchema, SealedChoiceSchema } from '@autorest/codemodel';
import { camelCase, deconstruct, fixLeadingNumber, pascalCase, minimum, getPascalIdentifier, serialize } from '@azure-tools/codegen';
import { values, Dictionary, length } from '@azure-tools/linq';
...
function setSchemaNames(schemaGroups: Dictionary<Array<Schema>>, azure: boolean, serviceNamespace: string) {
  ...
  for (const group of values(schemaGroups)) {
    for (const schema of group) {
      ...
      let n = 1;
      while (thisNamespace.has(schemaName) && !schema.language.csharp) {
        schemaName = getPascalIdentifier(`${details.name}_${n++}`);
      }
      thisNamespace.add(schemaName);

      // object types.
      if (schema.type === SchemaType.Object || schema.type === SchemaType.Dictionary || schema.type === SchemaType.Any || schema.type === SchemaType.AnyObject) {
